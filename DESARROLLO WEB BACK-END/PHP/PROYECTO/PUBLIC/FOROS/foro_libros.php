...
// Este bloque se ejecuta solo cuando JavaScript lo solicita para actualizar el chat en tiempo real.
if (isset($_GET['actualizar_mensajes']) && isset($_SESSION['json_asociado'])) {
    // Construye la ruta al archivo de mensajes del hilo actual, apuntando a la carpeta de LIBROS.
    $ruta_json = '../../ASSETS/JSON_MENSAJES/LIBROS/' . $_SESSION['json_asociado'];

    if (file_exists($ruta_json)) {
        // Si el archivo existe, lee su contenido y lo envía como respuesta en formato JSON.
...
if (isset($_POST['eliminar_hilo'])) {
    ...
    $ruta_hilos = '../../ASSETS/JSON/hilos.json'; // Archivo maestro que contiene todos los hilos.
    $hilos = [];
    ...
    // Ruta al archivo de mensajes del hilo a eliminar, en la carpeta LIBROS.
    $ruta_mensajes = '../../ASSETS/JSON_MENSAJES/LIBROS/' . $_SESSION['json_asociado'];
    ...
    // Guarda la lista de hilos actualizada. `array_values` reindexa el array.
...
$portal_cuenta = ""; // Controla si se muestra el perfil de usuario o los botones de login/registro.
$accion = ""; // Controla si el usuario tiene permisos para interactuar (crear hilos, enviar mensajes).
...
// (Lógica estándar presente en todas las páginas para mantener la sesión consistente)
...
if (isset($_POST['enviar'])) {
    ...
    // Ruta al archivo de mensajes del hilo actual, en la carpeta LIBROS.
    $ruta_json = '../../ASSETS/JSON_MENSAJES/LIBROS/' . $_SESSION['json_asociado'];
    ...
    if ($error == '') {
        ...
        header('Location: foro_libros.php');
        exit();
    }
}
?>
...
<head>
    ...
    <title>Foro de Libros - ForoConecta</title>
    <link rel="stylesheet" href="../../ASSETS/CSS/styles.css">
</head>
<script>
    // --- LÓGICA JAVASCRIPT PARA CHAT EN TIEMPO REAL ---

    // Función asíncrona que pide los mensajes al servidor y los muestra.
    async function cargarMensajes() {
        try {
            ...
            contenedor.innerHTML = ''; // Limpia el contenedor para evitar duplicados.

            // Itera sobre cada mensaje y crea su HTML.
...
<script>
    // Script para hacer scroll automático hacia abajo en el chat.
    ...
    <div class="page_container">
        <main>
            <div class="main_container_foro">
                <!-- Encabezado específico del Foro de Libros -->
                <div class="main_container_foro_6">
                    <div class="main_container_4">
                        <p> <i>Libros</i></p>
                        <div class="barra"></div>
                        <p class="p_1">Comparte la historia que todos quieren leer.</p>
                    </div>
                    ...
                    <div class="main_foro_container_1">
                        ...
                        <div class="main_foro_container_hilos_1">
                            ...
                            if (file_exists($ruta_json)) {
                                ...
                                if (is_array($hilos)) {
                                    // Bucle que muestra solo los hilos cuyo campo 'foro' es 'libros'.

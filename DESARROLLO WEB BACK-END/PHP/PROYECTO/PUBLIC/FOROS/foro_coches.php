...
// Este bloque se ejecuta cuando JavaScript solicita los mensajes más recientes de un hilo.
// NOTA: La ruta apunta a 'ANECDOTAS', aunque esta página es para el foro de 'Coches'. Es un error de copiado.
if (isset($_GET['actualizar_mensajes']) && isset($_SESSION['json_asociado'])) {
    // Construye la ruta al archivo de mensajes, apuntando a la carpeta de ANECDOTAS.
    $ruta_json = '../../ASSETS/JSON_MENSAJES/COCHES/' . $_SESSION['json_asociado'];
    ...
    if (file_exists($ruta_json)) {
        ...
        header('Content-Type: application/json'); // Informa al navegador que la respuesta es JSON.
        echo json_encode($mensajes, JSON_UNESCAPED_UNICODE);
        exit(); // Termina la ejecución para no enviar el resto del HTML.
    } else {
        // Si no encuentra el archivo, devuelve un código de error 404 (No encontrado).
...
// Se activa si se envía el formulario para eliminar un hilo.
if (isset($_POST['eliminar_hilo'])) {
    ...
    // Ruta al archivo de mensajes del hilo a eliminar, en la carpeta ANECDOTAS (inconsistencia).
    $ruta_mensajes = '../../ASSETS/JSON_MENSAJES/COCHES/' . $_SESSION['json_asociado'];

    // Filtra el array de hilos, conservando solo aquellos que NO coinciden con el que se va a eliminar.
    ...
    // Si el archivo de mensajes del hilo existe, se elimina del servidor.
    if (file_exists($ruta_mensajes)) {
        unlink($ruta_mensajes);
    }

    // Se elimina la variable de sesión para que el hilo ya no esté activo.
...
// Comprueba si el usuario está logueado a través de sesión o cookie.
...
if (isset($_SESSION['tiempo'])) {
    ...
    if ($vida_sesion > $inactividad) {
        // ... (código para marcar usuario como desconectado y destruir la sesión)
...
if (isset($_POST['enviar'])) {
    ...
    // Ruta al archivo de mensajes del hilo actual, en la carpeta ANECDOTAS (inconsistencia).
    $ruta_json = '../../ASSETS/JSON_MENSAJES/COCHES/' . $_SESSION['json_asociado'];
    ...
    if ($error == '') {
        ...
        // Redirige a la página de anécdotas, no a la de coches (inconsistencia).
        header('Location: foro_coches.php');
        exit();
    }
}
?>
...
<head>
    ...
    <title>Foro de Coches - ForoConecta</title>
    <link rel="stylesheet" href="../../ASSETS/CSS/styles.css">
</head>
<script>
    // --- LÓGICA JAVASCRIPT PARA CHAT EN TIEMPO REAL ---

    // Función asíncrona que pide los mensajes al servidor y los renderiza en la página.
    async function cargarMensajes() {
        try {
            // Realiza una petición GET al endpoint PHP definido al inicio.
            ...
            contenedor.innerHTML = ''; // Limpia el contenedor para evitar duplicar mensajes.
            ...
            console.error(e); // Muestra cualquier error en la consola del navegador.
        }
    }

    // Llama a la función 'cargarMensajes' cada 3 segundos para refrescar el chat.
    setInterval(cargarMensajes, 3000);
    // Carga los mensajes una vez al entrar en la página para no esperar 3 segundos.
    cargarMensajes();
</script>
<script>
    // Script para hacer scroll automático hacia el final del contenedor de mensajes.
    ...
    <div class="page_container">
        <main>
            <div class="main_container_foro">
                <!-- Encabezado específico del Foro de Coches -->
                <div class="main_container_foro_6">
                    <div class="main_container_4">
                        <p> <i>Coches</i></p>
                        <div class="barra"></div>
                        <p class="p_1">Comparte tus rutas y tu pasión por los coches.</p>
                    </div>
                    ...
                    <div class="main_foro_container_1">
                        ...
                        <div class="main_foro_container_hilos_1">
                            ...
                            if (file_exists($ruta_json)) {
                                ...
                                if (is_array($hilos)) {
                                    // Bucle que muestra los hilos. NOTA: La condición filtra por 'anecdotas', por lo que en esta página se mostrarán los hilos del foro de anécdotas, no los de coches.
                                    foreach ($hilos as $hilo) {
                                        if ($hilo['foro'] === 'coches') { // Esta es la inconsistencia principal. Debería ser 'coches'.
